...
El comando por sí solo no genera ninguna transformación visible en la base de datos, pero sí lo hace invisiblemente. Por eso, después de realizar las transformaciones necesarias a partir del `group_by` es necesario desagrupar para evitar posibles problemas futuros. Para esto se usa la función `ungruop()`.
